(window => {
   ...
   *    channel: 通道号
   *    videoCb: 视频解码回调，需要使用videoCb = Module.addFunction(fn)得到的返回值传入
   *    audioCb: 音频解码回调，需要使用audioCb = Module.addFunction(fn)得到的返回值传入
   * 2.发送原始音视频流
  ...
  class Decoder {
    ...
    addPlayerInstance(instance) {
      this.playerInstances[instance.channel] = instance;
    }
    // 开启channel的解码功能
    ...
    decodeMediaStream(stream) {
      ...
      Module.HEAPU8.set(stream, cacheBuffer);
      // totalSize += size;
    ...
    getVideoCallback() {
      ...
      if (this.videoCallback === null) {
        this.videoCallback = Module.addFunction(function (
          ...
          playerInst.displayVideoFrame.call(playerInst, obj);
        });
      }
